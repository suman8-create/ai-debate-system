...
class AuditResult(BaseModel):
  ...
  source_quality_score: float = Field(
      ge=0.0,
      le=10.0,
      description="Score evaluating domain and source credibility.",
  )
  logical_strength_score: float = Field(
      ge=0.0, le=10.0, description="Score measuring reasoning soundness."
  )
  relevance_score: float = Field(
      ge=0.0, le=10.0, description="Score measuring topic relevance."
  )
  rebuttal_quality_score: Optional[float] = Field(
      ...
      le=10.0,
      description=(
          "Score measuring how directly it counters the opponent (if"
          " applicable)."
      ),
  )
  detected_fallacies: List[FallacyType] = Field(
      default_factory=list, description="List of detected logical fallacies."
  )
  repetition_detected: bool = Field(
      default=False,
      description=(
          "Whether this argument simply repeats an earlier point in the debate."
      ),
  )
  verdict: str = Field(description="PASS, REVISE, or FORFEIT.")
