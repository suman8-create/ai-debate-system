...
def pro_generate_node(state: DebateState) -> Dict[str, Any]:
    ...
    # Visual Output
    attempt_str = f" (Revision Attempt {state.pro_revision_count + 1})" if state.pro_revision_count > 0 else ""
    print(f"\n{'='*25} PRO TURN [Round {state.current_round} - {pro_arg.argument_type}]{attempt_str} {'='*25}")
...
def pro_audit_node(state: DebateState) -> Dict[str, Any]:
    ...
    print(f"[Audit: PRO] Verdict: {verdict} | Logic Score: {audit.logical_strength_score} | Relevance: {audit.relevance_score}")
    if verdict != "PASS":
...
def con_generate_node(state: DebateState) -> Dict[str, Any]:
    ...
    # Visual Output
    attempt_str = f" (Revision Attempt {state.con_revision_count + 1})" if state.con_revision_count > 0 else ""
    print(f"\n{'='*25} CON TURN [Round {state.current_round} - {con_arg.argument_type}]{attempt_str} {'='*25}")
...
def con_audit_node(state: DebateState) -> Dict[str, Any]:
    ...
    verdict = audit.verdict.upper()
    print(f"[Audit: CON] Verdict: {verdict} | Logic Score: {audit.logical_strength_score} | Relevance: {audit.relevance_score}")
    if verdict != "PASS":
